    ...
    <main className="flex min-h-screen items-center justify-center px-6 py-10">
      <div className="grid w-full max-w-6xl overflow-hidden rounded-[32px] border border-white/10 bg-white/[0.05] xl:grid-cols-[0.95fr,1.05fr]">
        <section className="border-r border-white/10 bg-gradient-to-br from-cyan-500/10 via-sky-500/5 to-violet-500/10 p-8 md:p-12 xl:p-16">
          ...
          <p className="mt-4 text-sm leading-7 text-slate-400 md:text-base">Register your team, create a workspace, and start uploading logs into a premium observability platform with room for billing, sources, roles, and long-term scale.</p>
          <div className="mt-8 grid gap-3">
            {[
              "Auth + workspace onboarding included",
              "Premium landing and dashboard UI",
              "Real upload parsing and charts",
              "Team invites, source setup, and billing-ready workspace core"
            ].map((item) => <div key={item} className="rounded-2xl border border-white/10 bg-white/[0.04] p-4 text-sm text-slate-200">{item}</div>)}
          </div>
        </section>

        <section className="p-8 md:p-12 xl:p-16">
          <div className="badge"><span className="h-2 w-2 rounded-full bg-cyan-300 animate-pulse" /> Account setup</div>
          <form className="mt-6 space-y-4" action={(formData) => startTransition(async () => {
          ...
          <p className="mt-6 text-sm text-slate-400">Already have an account? <Link href="/login" className="text-cyan-300 hover:underline">Sign in</Link></p>
        </section>
      </div>
    </main>
  );
