...
const INSERT_BATCH_SIZE = 500; // smaller batches inside a transaction are safer
...
export async function ingestUpload(params: {
  ...
  // ✅ Wrap all batch inserts in a single transaction.
  // If any batch fails, none of the rows are committed — no partial data.
...
// ---------------------------------------------------------------------------
// Webhook fast-path: ingest pre-parsed events without re-parsing
// ---------------------------------------------------------------------------
