...
import { createAlertRuleAction, createAlertChannelAction, createApiSourceAction, createInviteAction, updateWorkspaceProfileAction } from "@/app/actions/workspace";

function ActionForm({ action, children }: { action: (formData: FormData) => Promise<{ ok: boolean; message: string }>; children: (pending: boolean, status: string, ok: boolean) => ReactNode }) {
  ...
  return (
    <form action={(formData) => startTransition(async () => {
      ...
      {children(pending, status, statusOk)}
    </form>
  );
}
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <select name="logStorageMode" className="input" defaultValue={workspace.logStorageMode}>
              ...
              <option value="s3-archive">S3 archive + DB metadata</option>
            </select>
          ...
          <div className="flex items-center gap-3">
            ...
            {status && <span className={`text-sm ${ok ? "text-cyan-300" : "text-rose-400"}`}>{status}</span>}
          </div>
        </div>
      )}
    </ActionForm>
      ...
      {(pending, status, ok) => (
        <div className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-[1.2fr,0.8fr,auto]">
            <input name="email" className="input" placeholder="member@company.com" type="email" />
      ...
      {(pending, status, ok) => (
        <div className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <select name="type" className="input" defaultValue="api"><option value="api">API</option><option value="s3">S3</option></select>
            <input name="endpointUrl" className="input" placeholder="Endpoint URL" type="url" />
            ...
            <input name="schedule" className="input" placeholder="Schedule (cron)" />
            <select name="authType" className="input" defaultValue="bearer">
              <option value="none">None</option><option value="bearer">Bearer</option><option value="basic">Basic</option><option value="iam">IAM/Role</option>
            </select>
          </div>
          <div className="flex items-center gap-3">
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            ...
            <select name="metric" className="input" defaultValue="errorRate">
              ...
              <option value="piiEvents">PII events</option>
            </select>
          ...
          <div className="flex items-center gap-3">
            <button className="btn-primary" disabled={pending}>{pending ? "Saving…" : "Create rule"}</button>
            {status && <span className={`text-sm ${ok ? "text-cyan-300" : "text-rose-400"}`}>{status}</span>}
          </div>
        </div>
      )}
...
export function AlertChannelForm({ workspaceId }: { workspaceId: string }) {
  return (
    <ActionForm action={createAlertChannelAction}>
      {(pending, status, ok) => (
        <div className="space-y-4">
          <input type="hidden" name="workspaceId" value={workspaceId} />
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <input name="name" className="input" placeholder="Channel name (e.g. Ops Slack)" />
            <select name="type" className="input" defaultValue="webhook">
              <option value="webhook">Webhook</option>
              <option value="slack">Slack</option>
              <option value="email">Email</option>
            </select>
            <input name="destination" className="input xl:col-span-2" placeholder="Webhook URL or email address" />
          </div>
          <div className="flex items-center gap-3">
            <button className="btn-primary" disabled={pending}>{pending ? "Saving…" : "Add channel"}</button>
            {status && <span className={`text-sm ${ok ? "text-cyan-300" : "text-rose-400"}`}>{status}</span>}
          </div>
        </div>
      )}
