  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <h1 className="mt-4 text-4xl font-semibold tracking-tight md:text-5xl">Compliance, masking, and tenancy posture</h1>
        <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-400 md:text-base">This version adds stronger auth handling, indexed security review, and PII/secret detection to uploaded logs.</p>
      </header>
      <div className="grid gap-5 xl:grid-cols-2">
        <Panel className="p-5 md:p-6">
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-2">
            {[
              ...
              ["Security events", securityStats.piiEvents],
              ["Monthly ingest limit", `${workspace.maxMonthlyIngestMb} MB`],
              ["Max members", workspace.maxUsers]
            ].map(([label, value]) => (
        ...
        <Panel className="p-5 md:p-6">
          <div className="text-lg font-semibold">Detection insights</div>
          <div className="mt-5 space-y-3">
            {metrics.topSignatures.map((item) => (
              <div key={item.signature} className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
                <div className="font-medium break-words">{item.signature}</div>
                <div className="mt-1 text-xs text-slate-400">Repeated {item.count} time(s)</div>
              </div>
            ))}
            {!metrics.topSignatures.length ? <div className="text-sm text-slate-400">No repeated warning/error signatures yet.</div> : null}
          </div>
        </Panel>
      </div>
      <Panel className="p-5 md:p-6">
