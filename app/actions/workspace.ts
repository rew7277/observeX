...
import { computeAuditHash, createOpaqueToken, sanitizeFreeText, isSafeHttpUrl } from "@/lib/security";
import { z } from "zod";
...
// FIX #7 — Audit log tamper protection: each event hashes over the previous one,
// so any deletion or modification breaks the chain and is detectable.
async function createAuditEvent(data: {
  workspaceId: string; userId: string; action: string; details?: string;
}) {
  // Get the most recent audit event's hash for chaining
  ...
  const now          = new Date();
  const chainHash    = computeAuditHash(previousHash, { ...data, createdAt: now });

  return db.auditEvent.create({
    data: { ...data, chainHash, createdAt: now },
  });
}
...
export async function updateWorkspaceProfileAction(formData: FormData) {
  ...
  await createAuditEvent({ workspaceId: parsed.data.workspaceId, userId: user.id, action: "workspace.updated", details: "Profile updated" });
...
export async function createInviteAction(formData: FormData) {
  ...
  // FIX #20 — Enforce maxUsers plan limit before creating an invite
  const workspace = await db.workspace.findUnique({
    where:  { id: parsed.data.workspaceId },
    select: { maxUsers: true },
  });
  if (workspace && workspace.maxUsers > 0) {
    const currentMembers = await db.membership.count({ where: { workspaceId: parsed.data.workspaceId } });
    if (currentMembers >= workspace.maxUsers) {
      return { ok: false, message: `Your plan allows a maximum of ${workspace.maxUsers} members. Upgrade to invite more.` };
    }
  }
  ...
  await db.invite.create({
    data: {
      workspaceId: parsed.data.workspaceId, email: parsed.data.email, role: parsed.data.role,
      createdById: user.id, token,
      expiresAt:   new Date(Date.now() + 1000 * 60 * 60 * 24 * 7),
    },
  });

  await createAuditEvent({ workspaceId: parsed.data.workspaceId, userId: user.id, action: "invite.created", details: `Invite for ${parsed.data.email} as ${parsed.data.role}` });
...
export async function createApiSourceAction(formData: FormData) {
  ...
  await db.sourceRun.create({
    data: { workspaceId: parsed.data.workspaceId, apiSourceId: source.id, status: "paused", errorMessage: "Health test pending." },
  });

  await createAuditEvent({ workspaceId: parsed.data.workspaceId, userId: user.id, action: "source.created", details: `${parsed.data.name} added` });
  await refreshWorkspace(parsed.data.workspaceId);
...
export async function createAlertRuleAction(formData: FormData) {
  ...
  await db.alertRule.create({
    data: {
      name: parsed.data.name, metric: parsed.data.metric,
      operator: parsed.data.operator, threshold: parsed.data.threshold,
      severity: parsed.data.severity, workspace: { connect: { id: parsed.data.workspaceId } },
    },
  });

  await createAuditEvent({ workspaceId: parsed.data.workspaceId, userId: user.id, action: "alert-rule.created", details: parsed.data.name });
  await refreshWorkspace(parsed.data.workspaceId);
...
export async function createAlertChannelAction(formData: FormData) {
  ...
  await createAuditEvent({ workspaceId: parsed.data.workspaceId, userId: user.id, action: "alert-channel.created", details: `${parsed.data.name} (${parsed.data.type})` });
  await refreshWorkspace(parsed.data.workspaceId);
